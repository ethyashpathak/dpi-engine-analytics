...
for (const auto& [app, count] : stats_.app_counts) {
    apps[appTypeToString(app)] = count;
}
j["applications"] = apps;
...
json threads;
for (size_t i = 0; i < lbs_.size(); i++) {
    threads["LB" + std::to_string(i)] = lbs_[i]->dispatched();
}
for (size_t i = 0; i < fps_.size(); i++) {
    threads["FP" + std::to_string(i)] = fps_[i]->processed();
}
j["threads"] = threads;
...
std::cout << "[INFO] stats.json generated\n";
